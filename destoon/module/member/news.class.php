...
class news {
	...
	function add($post) {
		...
		clear_upload($post['content'].$post['thumb'], $this->itemid, $this->table);
     	//修改添加资讯表
      	$data = DB::get_one("SELECT * FROM {$this->table} WHERE itemid = {$this->itemid}");
        ...
        $time = time();
        $table = DT_PRE.'article_21';
        $artSqlK = "`catid`,`level`,`title`,`style`,`thumb`,`username`,`addtime`,`editor`,`edittime`,`status`,`linkurl`,`note`,`islink`,`subtitle`";
        $artSqlV = "'{$catid[array_rand($catid)]}','{$data['level']}','{$data['title']}','{$data['style']}','{$data['thumb']}','{$data['username']}','{$data['addtime']}','{$data['editor']}','{$time}','{$data['status']}','{$data['linkurl']}','{$data['note']}','1',''";
        DB::query("INSERT INTO {$table} ($artSqlK) VALUES ($artSqlV)");
		return $this->itemid;
	}
