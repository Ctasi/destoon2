...
class sell {
	...
	function get_list($condition = 'status=3', $order = 'edittime DESC', $cache = '') {
		...
		$result = DB::query("SELECT * FROM {$this->table} WHERE $condition ORDER BY 'editdate desc,vip desc,edittime desc' LIMIT $offset,$pagesize", $cache);
	...
	function update($itemid) {
		...
		$linkurl = itemurl($item);
      	if(empty($linkurl)){
        	$linkurl = "show.php?itemid=".$item['itemid'];
        }
